...
exports.onLogCreated = functions.firestore
    ...
    try {
        ...
        if (recentLogs.docs.length > 1) {
            ...
            const hoursSinceLast = (Date.now() - previousLog.createdAt.toDate().getTime()) / 1000 / 60 / 60;
        ...
        if (allLogs.docs.length > 5) {
            const values = allLogs.docs.slice(1).map((d) => d.data().value);
        ...
        if (newStreak === 7)
            totalXP += XP_STREAK_7;
        if (newStreak === 30)
            totalXP += XP_STREAK_30;
        if (newStreak === 100)
            totalXP += XP_STREAK_100;
...
async function createFeedItem(userId, userData, itemData) {
    const feedItem = {
        type: itemData.type,
        actorId: userId,
...
async function checkBadges(userId, habitId, streak, habit) {
    const earnedRef = db.collection(`userBadges/${userId}/earned`);
    const awardBadge = async (badgeId, xpReward) => {
    ...
    // Streak badges
    if (streak >= 7)
        await awardBadge('on-fire', 50);
    if (streak >= 30)
        await awardBadge('unstoppable', 100);
    if (streak >= 100)
        await awardBadge('centurion', 250);
    // Total logs badges
    const totalLogs = (habit.totalLogs || 0) + 1;
    if (totalLogs >= 100)
        await awardBadge('century', 50);
    // Category-specific badges
    if (habitId === 'books' && totalLogs >= 10)
        await awardBadge('bookworm', 50);
    if (habitId === 'gym' && streak >= 7)
        await awardBadge('iron-week', 50);
    if (habitId === 'cold-shower' && totalLogs >= 10)
        await awardBadge('cold-warrior', 100);
    if (habitId === 'alcohol-free' && totalLogs >= 30)
        await awardBadge('sober', 100);
    if (habitId === 'early-wake' && streak >= 14)
        await awardBadge('5am-club', 100);
}
//# sourceMappingURL=onLogCreated.js.map